...
- (void)authorize:(WBAuthorize *)authorize didSucceedWithAccessToken:(NSString *)theAccessToken userID:(NSString *)theUserID expiresIn:(NSInteger)seconds{
    ...
    if([self.loginType isEqualToString:@"login"]&&_delegate&&[_delegate respondsToSelector:@selector(sinaLogin)]) {
        
        [_delegate sinaLogin];
    }
   else if (_delegate && [_delegate respondsToSelector:@selector(SinaLoginSuccess)])
    {
