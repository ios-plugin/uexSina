...
- (void)startAuthorize:(UIWebView*)inWebView
{
    _webView = inWebView;
    NSDictionary *params = [NSDictionary dictionaryWithObjectsAndKeys:appKey, @"client_id",
                                                                      @"code", @"response_type",
    ...
    NSMutableURLRequest *sendReq = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:urlString]];
	[_webView loadRequest:sendReq];
}
...
- (void)request:(WBRequest *)theRequest didFinishLoadingWithResult:(id)result
{
    ...
    if ([result isKindOfClass:[NSDictionary class]])
    {
        ...
        [ud setObject:remind_in forKey:@"remind_in"];
        [ud setObject:expires_in forKey:@"expires_in"];
